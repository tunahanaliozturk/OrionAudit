...
- 🚧 **Periodic snapshotting policy.** Optional opt-in: write a full `Snapshot` every N changes
  ...
  ≤ `asOf` and replays diffs from there — turns reconstruction into O(K) where K = changes since
  the last snapshot.
...
- **PRs welcome** for items marked 🚧 — coordinate via the issue first so we don't double-work.
