...
/// <see cref="AuditLog.PreviousHash"/> onto a batch of newly-captured rows before they are
/// persisted. Stateless and reflection-free so it is shared by the synchronous interceptor and the
/// async dispatcher and stays Native-AOT clean. The database read that supplies each stream's
/// current chain head lives in the caller (which owns the <c>DbContext</c>); this engine only does
/// the deterministic in-memory chaining.
/// </summary>
public static class AuditHashChainStamper
{
    ...
    /// <see cref="AuditHashChainScope.PerEntityStream"/> this is (EntityType, EntityId).
    /// </summary>
    public readonly record struct ChainKey(string EntityType, string EntityId);

    /// <summary>
    /// Computes the <see cref="ChainKey"/> for a row under the supplied scope.
    /// </summary>
    public static ChainKey KeyFor(AuditLog entry, AuditHashChainScope scope)
    {
        ArgumentNullException.ThrowIfNull(entry);
        // Only one scope today; the switch documents intent and gives a single edit point when a
        // second scope (e.g. per-tenant) is added without silently mis-keying existing chains.
        return scope switch
        {
            AuditHashChainScope.PerEntityStream => new ChainKey(entry.EntityType, entry.EntityId),
            _ => new ChainKey(entry.EntityType, entry.EntityId),
        };
    }
    ...
    /// <see cref="AuditLog.PreviousHash"/> and <see cref="AuditLog.EntryHash"/> are assigned.</param>
        ...
        AuditHashChainScope scope)
    {
        ...
        {
            var key = KeyFor(row, scope);
            if (!runningHead.TryGetValue(key, out var previous))
            {
                previous = existingHeadHashes.TryGetValue(key, out var head) ? head : null;
            }

            row.PreviousHash = previous;
            row.EntryHash = AuditEntryHasher.ComputeEntryHash(row, previous);
            runningHead[key] = row.EntryHash;
        }
    }

    /// <summary>
    /// Returns the distinct <see cref="ChainKey"/>s present in <paramref name="newRows"/> under the
    /// supplied scope, so the caller can fetch exactly those streams' current head hashes from the
    /// store in one query.
    /// </summary>
